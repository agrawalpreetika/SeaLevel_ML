...
# Load the dataset
df = pd.read_csv('datasets_csv/Antarctic Extent.csv') 

# Drop columns that are not needed
df = df.drop(columns=['Annual','Unnamed: 13','Unnamed: 15','Unnamed: 16', 'Unnamed: 17'])

# Melt the DataFrame from wide to long format
melted_df = df.melt(id_vars=['Year'], var_name='Month', value_name='sh_extent')
...
output_file_path = 'processed_csv/processed_SHextent.csv'  # replace with desired output path
